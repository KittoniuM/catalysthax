...
HRESULT CD3DBaseRender::Initialize(IDirect3DDevice9 *pD3Ddev)
{
	...
	{
		_DEBUGad("m_pD3Ddev is NULL");
		return E_FAIL;
	}
	
	if( !m_statesOK && FAILED(CreateStates()) )
	{
		_DEBUGad("CreateStates() failed");
		return E_FAIL;
	}
...
HRESULT CD3DBaseRender::BeginRender()
{
	if( !m_statesOK )
	{
		_DEBUGad("::BeginRender() m_statesOK = false");
		return E_FAIL;
	}
...
HRESULT CD3DBaseRender::EndRender()
{
	if( !m_statesOK )
	{
		_DEBUGad("::EndRender() m_statesOK = false");
		return E_FAIL;
	}
...
HRESULT CD3DFont::Initialize(IDirect3DDevice9 *pD3Ddev)
{
	...
	{
		_DEBUGad("::Initialize(pD3Ddev) failed");
		return E_FAIL;
	}

	
		if((m_pRender = new CD3DRender(16)) == NULL)
		{
			_DEBUGad("m_pRender = NULL");
			return E_FAIL;
		}
	...
	{
		_DEBUGad("m_pRender->Initialize failed");
		return E_FAIL;
	}
	...
	{
		_DEBUGad("CreateVertexBuffer failed");
...
HRESULT CD3DFont::Print(float x, float y, DWORD colour, char *szText, DWORD dwFlags)
{
	if(!m_isReady)
	{
		_DEBUGad("::Print() m_isReady = false");
		return E_FAIL;
	}
	...
	{
		_DEBUGad("::Print() BeginRender() failed");
		return E_FAIL;
	}
	...
	if( dwFlags & FT_BORDER )
	{
		...
		{
			D3DAddQuad(m_pRender, x-1.0f, y-1.0f, strWidth+2.0f, m_fChrHeight+2.0f, 0x40000000);
			m_pRender->End();
		}		
	}
