...
{
//	add_log("Create Direct3D9 device");
	_asm{
		nop;nop;nop;nop;
	}
	extern CMenu *pMenu;
	...
	//	HookinitDLL((CClassManager **)0x24e0024);
		RetourFunc( (BYTE*)GetProcAddress(GetModuleHandle("d3d9"), "Direct3DCreate9"), (BYTE*)oDirect3DCreate9, 5);

	//	while(GetAsyncKeyState(VK_END) == 0)
	//	{
	//		Sleep(5);
	//	}
		pMenu = new CMenu		(ret->m_pD3Ddev);
