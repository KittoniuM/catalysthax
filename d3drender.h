...
{
public:
	CD3DFont(char *szFontName, int fontHeight, int maxCharacters, DWORD dwCreateFlags = 0x0);
	~CD3DFont();

	HRESULT Initialize(IDirect3DDevice9 *pD3Ddev);
	HRESULT Invalidate();

	HRESULT Print(float x, float y, DWORD colour, char *szText, DWORD dwFlags = 0x0);

	float DrawLength(char*) const;
